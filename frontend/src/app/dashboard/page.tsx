  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-[1440px] space-y-5 px-6 py-5">
        ...
        {error && data && (
          <div className="flex items-center gap-3 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-2.5 text-sm text-red-300">
            <span>⚠</span>
            <span className="flex-1">{error} — retrying…</span>
          </div>
        )}
            ...
            {data.pending_signals.length === 0 ? (
              ...
              <div className="space-y-4">
                {data.pending_signals.map((s, i) => {
                  ...
                  const rr = slDist > 0 ? Math.abs(s.take_profit - s.entry_price) / slDist : 0;
                  return (
                    <div
                      key={s.signal_id || i}
                    ...
                    >
                      {/* Header: type + direction + confidence */}
                      ...
                      <div className="grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-lg bg-surface-100 px-2 py-1.5">
                      ...
                      <div className="flex items-center gap-2 text-[11px]">
                        <span className="rounded bg-surface-200 px-1.5 py-0.5 text-slate-400">
                          {rr.toFixed(1)}:1 R:R
                        </span>
                        <span className="rounded bg-surface-200 px-1.5 py-0.5 text-slate-400">
                          {s.lot_size} contract{s.lot_size !== 1 ? "s" : ""}
                        </span>
                        <span className="rounded bg-surface-200 px-1.5 py-0.5 text-slate-400">
                          SL {slDist.toFixed(1)} pts
                        </span>
                      </div>
                      ...
                      {/* Analysis Thesis */}
                      {s.thesis && (
                        <div className="rounded-lg border border-white/[0.06] bg-surface-100 p-3">
                      ...
                      {s.confluence_factors && s.confluence_factors.length > 0 && (
                        <div>
                          <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">
                            Confluence ({s.confluence_factors.length})
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            {s.confluence_factors.map((f, fi) => (
                              <span
                                key={fi}
                                className="inline-flex items-center gap-1 rounded-md bg-brand/10 px-2 py-0.5 text-[10px] text-brand/80"
                              >
