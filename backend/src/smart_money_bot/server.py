...
from smart_money_bot.models.schemas import DashboardState, SessionPhase
...
_tradovate_ready = bool(_cfg.username and _cfg.password)
...
async def _candle_feed_loop() -> None:
    """
    Every 10 seconds:
    1. Pull live candles from Tradovate WS buffer
    2. If empty, request chart data via REST fallback
    3. Feed them into the orchestrator engines
    4. Update the account state from Tradovate API
    """
        ...
        try:
            if _is_live and isinstance(broker, TradovateBroker):
                ...
                if (not candles or len(candles) == 0) and not _rest_fetch_done:
                    ...
                    try:
                        candles = await broker.fetch_candles_rest(count=500)
                        if candles:
...
async def get_positions():
    """Live positions from Tradovate."""
    if _is_live and isinstance(broker, TradovateBroker):
        positions = await broker.get_positions()
        return {"source": "tradovate", "positions": positions}
    return {"source": "paper", "positions": []}
...
async def change_timeframe(req: TimeframeRequest):
    ...
    if not (_is_live and isinstance(broker, TradovateBroker)):
        return {"ok": False, "error": "Not connected to Tradovate"}

    underlying_type, element_size = _TIMEFRAME_MAP[tf]
    ok = await broker.change_chart_timeframe(element_size, underlying_type)
    return {
        "ok": ok,
        "timeframe": tf,
...
@app.get("/api/candles")
async def get_candles(limit: int = 1000):
    """OHLCV candle data for charting."""
    if _is_live and isinstance(broker, TradovateBroker):
        bars = broker.candles[-limit:] if broker.candles else []
    ...
    # Fallback: use orchestrator candle cache
    if orchestrator._candle_cache:
        bars = orchestrator._candle_cache[-limit:]
        return {
            "source": "engine_cache",
            "symbol": "NAS100",
            "count": len(bars),
...
async def _auto_trade_loop() -> None:
    """Continuous scan → execute loop. Runs every ~30 seconds."""
    global _auto_trade_stats
    log.info("▶ Auto-trade loop STARTED")
    while True:
        try:
            ...
            if _is_live and isinstance(broker, TradovateBroker):
                ...
                # Paper mode — just run scan for signals display
                orchestrator.scan()
    ...
    try:
        ...
        return {
            ...
            "hint": (
                ...
                "then retry. Or provide API keys (cid + sec)."
            ),
        }
