"""
╔══════════════════════════════════════════════════════════════════════╗
║      NQ-TRADING AGENTS — MASTER CONFIGURATION                         ║
...
# Load .env from project root (secrets stay out of source code)
try:
    ...
    pass  # python-dotenv not installed yet — keys must be in shell env
...
# ─────────────────────────────────────────────────────────────────────
#  AI ADVISORY — MULTI-AGENT LLM INTEGRATION
# ─────────────────────────────────────────────────────────────────────
@dataclass(frozen=True)
class AIAdvisoryConfig:
    """Configuration for the multi-agent AI advisory layer."""
    enabled: bool = True                          # AI advisory is ON by default
    llm_provider: str = "google"                  # openai, anthropic, google, ollama
    deep_think_model: str = "gemini-2.0-flash"    # Gemini Flash for complex reasoning
    quick_think_model: str = "gemini-2.0-flash"   # Gemini Flash for quick tasks
    max_debate_rounds: int = 1                    # keep low for intraday speed
    max_risk_discuss_rounds: int = 1
    selected_analysts: tuple[str, ...] = ("market", "news")  # skip social/fundamentals for futures
    require_ai_approval: bool = False             # True = AI can veto signals (Tier 2)
    confidence_threshold: float = 0.6             # min AI confidence to approve
    timeout_seconds: int = 30                     # max time for AI deliberation
    ticker_symbol: str = "NQ=F"                   # Yahoo Finance NQ futures symbol
    # External data settings
    fetch_external_news: bool = True              # pull live news via yfinance
    fetch_external_fundamentals: bool = True      # pull QQQ/VIX fundamentals
    data_vendor: str = "yfinance"                 # "yfinance" (free) or "alpha_vantage" (needs key)
    news_lookback_days: int = 3                   # how many days of news to fetch
    nq_proxy_tickers: tuple[str, ...] = ("QQQ", "^VIX", "^TNX")  # macro proxies
...
class Config:
    ...
    ai_advisory: AIAdvisoryConfig = field(default_factory=AIAdvisoryConfig)
    symbol: str = "NAS100"
