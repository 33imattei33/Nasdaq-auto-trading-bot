"""
╔══════════════════════════════════════════════════════════════════════╗
║      NQ-TRADING AGENTS — FASTAPI SERVER                                ║
║                                                                      ║
...
from nq_trading_agents.models.schemas import DashboardState, SessionPhase, CandleData, TradeRecord, TradeStatus, TradeDirection
...
from nq_trading_agents.infrastructure.brokers.tradovate_broker import (
    TradovateBroker,
    TradovateConfig,
)
...
async def generate_blog_article(req: BlogGenerateRequest):
    """
    Generate an SEO-optimized blog article about trading topics.
    Uses structured templates enriched with domain knowledge.
    """
    import random

    kw = req.keyword.strip() or req.topic.split()[0:3]
    kw_str = req.keyword.strip() if req.keyword.strip() else " ".join(req.topic.split()[:3])

    # Determine target word count
    target = 1000
    if "600" in req.length or "Short" in req.length:
        target = 600
    elif "1500" in req.length or "Long" in req.length:
        target = 1500
    elif "2000" in req.length or "In-Depth" in req.length:
        target = 2000
    ...
    # Pick best matching intro
    intro = intros["default"]
    for key, text in intros.items():
    ...
    if req.tone == "Professional":
        tone_adj = "From a professional trading perspective, "
    elif req.tone == "Conversational":
        tone_adj = "Let's be honest — "
    elif req.tone == "Technical":
        tone_adj = "From a technical analysis standpoint, "
    elif req.tone == "Persuasive":
        tone_adj = "If you're serious about consistent profitability, "

    sections = [
        ("Introduction", f"{tone_adj}{intro}"),
        (f"What Is {kw_str.title()}?",
         f"{kw_str.title()} refers to a specific concept within institutional trading that separates profitable traders from the rest. "
         f"In the context of NAS100 futures, this involves understanding how large players position themselves before major price moves.\n\n"
         f"The key insight is that markets are not random — they follow predictable patterns driven by order flow, liquidity, and institutional positioning. "
         f"When you understand {kw_str}, you begin to see the market through the lens of the participants who actually move price."),
        (f"Why {kw_str.title()} Matters for NAS100 Traders",
         f"The NAS100 (Nasdaq-100 E-mini futures) is one of the most liquid and volatile instruments available to retail traders. "
         f"This volatility creates both opportunity and risk. Without understanding {kw_str}, traders are essentially gambling.\n\n"
         f"Here's why this matters:\n\n"
         f"- **Institutional footprints** are visible in price action if you know what to look for\n"
         f"- **Kill zones** (London 08-12 UTC, New York 14-16 UTC) concentrate 80% of the daily range\n"
         f"- **Stop hunts** occur before nearly every major reversal — they're a feature, not a bug\n"
         f"- **Liquidity pools** above swing highs and below swing lows attract institutional orders"),
        (f"How to Apply {kw_str.title()} in Practice",
         f"Applying {kw_str} requires a systematic approach. Here is a step-by-step framework:\n\n"
         f"1. **Identify the weekly structure** — Is Monday setting a Connector? Is it Tuesday accumulation day?\n"
         f"2. **Wait for the kill zone** — Only take trades during London or New York sessions\n"
         f"3. **Scan for the Signature Trade** — Look for the 4-step pattern: Wedge → Stop Hunt → Exhaustion → Reversal\n"
         f"4. **Confirm with confluence** — Check market structure, liquidity zones, and wick analysis\n"
         f"5. **Execute with APEX-compliant risk** — $300 max risk, 2% equity cap, 4-contract limit\n\n"
         f"This framework ensures you're trading with the institutions, not against them."),
        ("Risk Management: The Non-Negotiable",
         f"No discussion of {kw_str} is complete without addressing risk management. Our APEX-compliant risk engine enforces:\n\n"
         f"- **$300 maximum risk per trade** — protects against black swan events\n"
         f"- **2% equity cap** — never risk more than 2% of your account on a single position\n"
        ...
        (f"Common Mistakes With {kw_str.title()}",
         ...
         f"- **Ignoring the weekly structure** — Each day has a role. Don't fight the narrative.\n"
         f"- **Chasing stop hunts** — The stop hunt is the trap, not the trade. Wait for the exhaustion.\n"
         f"- **Over-leveraging** — More contracts ≠ more profit. It means more risk.\n"
         f"- **Skipping confluence** — A signal without confluence is just a guess."),
        ("Conclusion",
    ...
    # Trim sections if short article requested
    if target <= 600:
        sections = [sections[0], sections[1], sections[3], sections[-1]]
    elif target <= 1000:
        sections = [sections[0], sections[1], sections[2], sections[3], sections[-1]]
    ...
    meta = f"Learn how {kw_str} works in NAS100 futures trading. Discover institutional strategies, kill zone timing, and APEX risk management in this comprehensive guide."
    if len(meta) > 160:
        meta = meta[:157] + "..."
...
async def get_dashboard():
    ...
    if _is_live and isinstance(broker, TradovateBroker):
        ...
        # Backfill trade_history from Tradovate fills when orchestrator has none
        if not state.trade_history:
            try:
                fills_raw = await broker.get_fills()
                if fills_raw:
                    for f in fills_raw[-20:]:
                        direction = TradeDirection.BUY if f.get("action") == "Buy" else TradeDirection.SELL
                        state.trade_history.append(TradeRecord(
                            trade_id=f"TV-{f.get('id', 0)}",
                            symbol=CONFIG.symbol,
                            direction=direction,
                            entry_price=f.get("price", 0.0),
                            stop_loss=0.0,
                            take_profit=0.0,
                            lot_size=float(f.get("qty", 1)),
                            status=TradeStatus.CLOSED,
                            signal_type="TRADOVATE_FILL",
                        ))
            except Exception as e:
                log.debug(f"Fill backfill failed: {e}")
...
async def _auto_trade_loop() -> None:
    ...
    _SIGNAL_COOLDOWN_SECONDS = 120  # 2-minute cooldown between ANY signals
    while True:
        try:
            _auto_trade_stats["scans"] += 1

            # Cooldown check — skip if last signal was too recent
            now = datetime.now(timezone.utc)
            if _last_signal_time and (now - _last_signal_time).total_seconds() < _SIGNAL_COOLDOWN_SECONDS:
            ...
            if _is_live and isinstance(broker, TradovateBroker):
                ...
                if signal:
                    # Always set cooldown when a signal fires (prevents spam)
                ...
                if signal:
                    ...
                    from nq_trading_agents.domain.entities.trade import Trade, TradeSide
...
async def test_trade():
    ...
    from nq_trading_agents.models.schemas import TradeRecord, TradeStatus, TradeDirection, SignalType
    trade = TradeRecord(
        trade_id=tid,
        symbol="NAS100",
...
async def close_all_positions():
    ...
    if not (_is_live and isinstance(broker, TradovateBroker)):
        from nq_trading_agents.models.schemas import TradeStatus
        closed_count = 0
        last_price = 0.0
        if orchestrator._candle_cache:
            last_price = orchestrator._candle_cache[-1].close
        for trade in orchestrator._trade_history:
            if trade.status == TradeStatus.OPEN:
                ...
                # Record outcome so AI agents can learn
                orchestrator.record_trade_outcome(trade.trade_id, trade.pnl)
                closed_count += 1
        if closed_count > 0:
...
async def browser_login(req: BrowserLoginRequest):
    ...
    from nq_trading_agents.infrastructure.brokers.browser_auth import (
...
async def get_ai_advisory():
    """Current state of the AI advisory engine."""
    state = orchestrator._get_ai_advisory_state()
    return state.model_dump()


class AIAdvisoryToggle(BaseModel):
    enabled: bool
    require_approval: bool = False


@app.post("/api/ai-advisory/toggle")
async def toggle_ai_advisory(req: AIAdvisoryToggle):
    """Toggle the AI advisory engine on/off at runtime.

    Note: This creates a new config since AIAdvisoryConfig is frozen.
    """
    from dataclasses import replace
    new_ai_cfg = replace(
        CONFIG.ai_advisory,
        enabled=req.enabled,
        require_ai_approval=req.require_approval,
    )
    # We can't mutate frozen Config, so update the module-level reference
    import nq_trading_agents.config as cfg_mod
    new_config = replace(cfg_mod.CONFIG, ai_advisory=new_ai_cfg)
    cfg_mod.CONFIG = new_config
    # Also update the orchestrator's reference
    orchestrator.ai_advisory._cfg = new_ai_cfg

    mode = "disabled"
    if req.enabled and req.require_approval:
        mode = "gate (AI can veto)"
    elif req.enabled:
        mode = "advisory (AI advises, doesn't block)"

    log.info(f"AI Advisory toggled: enabled={req.enabled}, require_approval={req.require_approval}, mode={mode}")
    return {"enabled": req.enabled, "require_approval": req.require_approval, "mode": mode}


@app.get("/api/ai-advisory/memory")
async def get_ai_memory():
    """View the AI advisory engine's trade memory/learning."""
    mem = orchestrator.ai_advisory._trade_memory
    return {
        "trade_count": len(mem),
        "recent_trades": mem[-20:],
        "formatted": orchestrator.ai_advisory._format_memory(),
    }


# ═══════════════════════════════════════════════════════════════════════
#  AGENTS OFFICE — Real-time pipeline event streaming
# ═══════════════════════════════════════════════════════════════════════

@app.get("/api/agents/events")
async def agents_sse():
    """Server-Sent Events stream for real-time agent pipeline monitoring.

    The frontend Agents Office page subscribes to this endpoint to
    display live status of each pipeline stage as agents execute.
    """
    from nq_trading_agents.engines.ai_advisory import pipeline_events

    async def event_generator():
        q = pipeline_events.subscribe()
        try:
            # Send recent history for hydration
            for evt in pipeline_events.history():
...
async def agents_history():
    """Return recent pipeline event history (for page load hydration)."""
    from nq_trading_agents.engines.ai_advisory import pipeline_events
    return {"events": pipeline_events.history()}
